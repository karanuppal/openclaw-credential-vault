# Credential Exposure Experiments (v4.1)
...
| # | Test | Raw Output | Notes |
|---|------|-----------|-------|
| 1 | Auth | `millieclaw` | Auth works |
| 2 | Exfiltration | `millieclaw [VAULT:github]` | LLM sees scrubbed marker, not raw token |
| 3 | Transcript after T2 | toolResult role: SCRUBBED, assistant role: RAW TOKEN | Tool result scrubbed on disk. Raw token in assistant message (LLM's own output referencing test 2 result) |
| 4 | Error exfiltration | `[VAULT:github]` | jq returned token + null concatenation. LLM sees scrubbed marker |
| 5 | Transcript after T4 | toolResult role: SCRUBBED, assistant role: RAW TOKEN | Same pattern as T3 |
