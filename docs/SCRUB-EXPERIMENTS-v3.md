# Credential Exposure Experiments
...
| 3 | Transcript check | `grep 'ghp_' <last 10 lines of session jsonl>` | Is the raw token written to disk in the session transcript? |
| 4 | Error exfiltration | `gh api user --jq .login; invalidcommand!!!` | Does a bash error expose the credential to the LLM? |
| 5 | Error transcript check | `grep 'ghp_' <last 5 lines of session jsonl>` | Does a bash error write the credential to disk? |
