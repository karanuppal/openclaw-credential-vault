# Credential Exposure Experiments (v5.2)
...
## message_sending Hook Investigation

### Findings (from planning agent + independent reviewer)

The `message_sending` hook IS registered, IS called, and DOES scrub correctly. The code is sound:
- `scrubTextWithTracking` correctly resets `lastIndex = 0` before `replace()` (no regex bug)
- Hook registered at priority 1 via `api.on("message_sending", ...)`
- `applyMessageSendingHook` in OpenClaw's delivery pipeline runs before message chunking
- Returns `{content}` only when scrubbing changed the text; `void` means no modification (correct per hook contract)

### Root cause of the GWS client ID leak to Telegram

The leak was NOT a hook failure. It was the LLM paraphrasing/quoting the credential in its own words:

1. **LLM saw the raw credential** in tool output (before `tool_result_persist` scrubbed the transcript copy)
2. **LLM wrote a partial reference** like `859385208636-...` or `859385208636-igvdh0067s50holelfgne4jdlg1roftt` in its response
3. **The scrub pattern requires the FULL string** including `.apps.googleusercontent.com` — partial forms don't match
4. **`message_sending` hook ran but found no match** — the partial credential passed through unscrubbed

### Why the scrub pattern was insufficient

The regex `859385208636-igvdh0067s50holelfgne4jdlg1roftt\.apps\.googleusercontent\.com` only matches the exact full credential. It does NOT match:
- Truncated forms: `859385208636-...`
- Partial forms: `859385208636-igvdh0067s50holelfgne4jdlg1roftt` (without the domain)
- Escaped forms: `859385208636-igvdh0067s50holelfgne4jdlg1roftt\.apps\.` (from tools.yaml display)

### Mitigations

1. Add shorter scrub patterns for unique prefixes (e.g., `859385208636-[a-zA-Z0-9]+`)
2. Consider auto-generating prefix patterns from literal credential values
3. The `credentialCache` literal scrub WOULD catch the full form, but partial quotes bypass it too

### GWS tokens purged

All three GWS credentials (`gws-oauth`, `gws-client-id`, `gws-client-secret`) have been:
- Removed from `tools.yaml`
- `.enc` files deleted from `~/.openclaw/vault/`
- Already gone from `/var/lib/openclaw-vault/`
...
2. Add Perl stdout scrubber to pipe subprocess output through credential replacement before exec captures it
3. Comprehensively test all scenarios with the scrubber
4. Document final architecture and update threat model
